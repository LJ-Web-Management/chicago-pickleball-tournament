...
const { requireUserOrAdmin } = require('../../lib/auth');
const { withHandler, HttpError, methodGuard } = require('../../lib/http');
const { DIVISIONS } = require('../../config/tournament');
const { buildRoundRobinSchedule } = require('../../lib/schedule');
const { recordResult } = require('../../lib/bracket');
...
module.exports = withHandler(async (req, res) => {
  ...
  if (stage === 'round_robin') {
    const template = buildRoundRobinSchedule(division).find((m) => m.match_key === matchKey);
  ...
  const result = await recordResult(division, stage, matchKey, sets, updatedBy);
