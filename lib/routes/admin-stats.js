...
const { getSettings, playerCap, teamCap } = require('../settings');
...
module.exports = withHandler(async (req, res) => {
  ...
  const divisions = settings.divisionOrder.map((key) => ({
    division: key,
    label: settings.divisions[key].label,
    players: playerMap[key]?.c || 0,
    playerCap: playerCap(settings, key),
    playersPaid: playerMap[key]?.paid_c || 0,
    teams: teamMap[key] || 0,
    teamCap: teamCap(settings, key),
  }));
