...
const { getSettings, playerCap } = require('../settings');
...
module.exports = withHandler(async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    `;
    if (countRows[0].c >= playerCap(settings, division)) {
      throw new HttpError(409, `Registration is full for the ${settings.divisions[division].label} division`);
    }
