...
async function getRoundRobinMatches(divisionKey) {
  const template = buildRoundRobinSchedule(divisionKey);
...
async function computeStandings(divisionKey) {
  ...
  const matches = await getRoundRobinMatches(divisionKey);

  const pools = [];
  for (let poolIndex = 0; poolIndex < div.courts; poolIndex++) {
    const teamNumbers = teamNumbersForPool(divisionKey, poolIndex).filter((t) => existingTeams.has(t));
...
function buildBracketMatches(divisionKey, seededTeamNumbers) {
  const div = DIVISIONS[divisionKey];
  ...
  for (let round = 1; round <= totalRounds; round++) {
    ...
    roundMatches.forEach((m, idx) => {
      ...
      m.start_time = addMinutes(TOURNAMENT.eliminationStart, m.slot_index * TOURNAMENT.matchSlotMinutes);
    });
...
async function generateBracket(divisionKey, force = false) {
  const div = DIVISIONS[divisionKey];
  ...
  const pools = await computeStandings(divisionKey);
  const advance = TOURNAMENT.advancePerPool;
  ...
  const matches = buildBracketMatches(divisionKey, seededTeamNumbers);
...
async function recordResult(divisionKey, stage, matchKey, sets, updatedBy) {
  ...
  const template = buildRoundRobinSchedule(divisionKey).find((m) => m.match_key === matchKey);
