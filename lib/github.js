...
// the raw CDN caches responses for 5 minutes (Cache-Control: max-age=300)
// regardless of any cache-busting on our end, which would make an admin's
// settings save look like it silently didn't take effect. The Contents API
// isn't behind that CDN, so a write is visible on the very next read.
async function readFile(path) {
  const { repo, branch, token } = repoInfo();
  const headers = { 'User-Agent': 'chicago-pickleball-tournament-app', Accept: 'application/vnd.github+json' };
  if (token) headers.Authorization = `Bearer ${token}`;
  const res = await fetch(`https://api.github.com/repos/${repo}/contents/${path}?ref=${branch}`, {
    headers,
    cache: 'no-store',
  });
  if (!res.ok) throw new Error(`GitHub contents fetch failed for ${path}: ${res.status}`);
