...
function getSqlInstance() {
  if (!sqlInstance) {
    ...
    // fullResults matches the { rows } shape the rest of this codebase uses
    // (neon's default is to return the row array directly, unlike pg/@vercel/postgres).
    sqlInstance = neon(conn, { fullResults: true });
  }
  return sqlInstance;
}
...
async function ensureSchema() {
  ...
  schemaReady = attempt.catch((err) => {
    schemaReady = null; // don't wedge future requests behind a transient failure
    throw err;
  });
  return schemaReady;
}
